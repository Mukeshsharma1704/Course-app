...
function AdminLogin() {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-100 font-sans relative overflow-hidden flex items-center justify-center px-6">
      ...
      <header className="absolute top-0 left-0 w-full flex justify-between items-center p-8 z-20">
        ...
        <div className="flex items-center gap-4">
          <Link to="/admin/signup" className="text-sm font-bold text-slate-400 hover:text-white transition-colors">
            Register
          </Link>
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="bg-slate-900/40 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-10 shadow-2xl">
          <div className="text-center mb-10">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-[10px] font-black uppercase tracking-widest mb-4">
              <FaShieldAlt /> Secure Admin Entry
            </div>
            <h2 className="text-3xl font-black text-white tracking-tight">Welcome Back.</h2>
            <p className="text-slate-500 text-sm font-medium mt-2">Initialize your administrative session.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label className="block text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 ml-1">Access Token (Password)</label>
            ...
            <button
              ...
              className="w-full bg-blue-600 hover:bg-blue-500 text-white py-4 rounded-2xl font-black text-lg transition-all shadow-lg shadow-blue-900/20 flex items-center justify-center gap-3 group active:scale-[0.98]"
            >
              {isSubmitting ? (
                <div className="w-6 h-6 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
              ) : (
                <>
                  Enter Dashboard <FaArrowRight className="group-hover:translate-x-1 transition-transform" />
                </>
              )}
            </button>
          </form>
