...
export const updateCourse = async (req, res) => {
  ...
  try {
    ...
    let imageData = {
      public_id: courseSearch.image?.public_id,
      url: courseSearch.image?.url,
    };
...
export const getCourses = async (req, res) => {
  try {
    // Use lean() to optimize query performance and select only needed fields
    const courses = await Course.find({})
      .select('title description price image createdAt')
      .lean()
      .limit(50);
